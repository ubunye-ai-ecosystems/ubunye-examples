...
# MAGIC %md
# MAGIC # Structured ingestion — Unity Catalog tables + SQL
# MAGIC
# MAGIC Reads `samples.bakehouse` (ships with every workspace), aggregates revenue by franchise and by product, and writes two Delta tables — one with `merge` (safe to re-run), one with `overwrite_partitions` (safe to backfill).
# MAGIC
...
if not task_dir:
    ...
    task_dir = str(Path("/Workspace") / here.relative_to("/") / "../pipelines/retail/ingestion/customer_revenue")
...
for t in ["franchise_revenue", "product_revenue"]:
